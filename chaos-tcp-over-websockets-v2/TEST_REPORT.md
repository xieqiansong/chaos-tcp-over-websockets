...
- 被测对象：
  - `tunnel_v1`：隧道 client v1 入口（本地端口 21001）
  - `tunnel_v2`：隧道 client v2 入口（本地端口 21002，连接池 + 会话绑定固定 WS 通道 + 零拷贝 `retainedDuplicate`）
...
- **v2 全面优于 v1**：同包大小下吞吐约为 v1 的 1.3～2.5 倍，RTT 更低。连接池（round-robin 分配、会话绑定固定 WS 通道、通道间隔离）与首包缓冲（会话未就绪时缓存而非丢弃）生效。
- **v2 随包增大优势放大**：1KB→1MB 吞吐 13→592 MB/s（约 44×）；v1 仅 10→245 MB/s（约 23×）。小包受 WS 帧/系统调用开销拖累，大包批处理收益最大。
- **1MB 档 v2(592) 已接近直连(888)**；64KB/256KB 区间与直连差距在 3～4× 内。v1 始终差一个数量级。
