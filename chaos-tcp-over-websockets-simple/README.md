# chaos-tcp-over-websockets-simple

基于 [Netty](https://netty.io/) 实现的 **TCP over WebSocket** 隧道工具（**简单直连版**），可在网络受限的环境下通过 WebSocket 协议中转任意 TCP 流量（如 SSO 数据库、内网服务等）。

> **模块定位**：本模块属于 `chaos-tcp-over-websockets` 项目集合中的 **simple（简单直连）** 路线 —— 目标地址由 URL 指定、单条 WS 连接、裸二进制帧透传，一条命令即可跑起来。与面向高并发、连接池 + 会话多路复用的兄弟模块 [`chaos-tcp-over-websockets-multiplexed`](../chaos-tcp-over-websockets-multiplexed/README.md) 是并列关系，连接数不高、追求快速部署时优先选本模块。
...
- Spring Boot 启动，构建产物为可执行 jar：`tcp-over-websockets-simple-exec.jar`。
...
java -jar target/tcp-over-websockets-simple-exec.jar server 7002

# 2) client 端：本地监听 13306，隧道转发到远端目标 127.0.0.1:3306
java -jar target/tcp-over-websockets-simple-exec.jar client 13306 ws://server-host:7002/forward/127.0.0.1/3306
```
...
隧道转发路径上的核心开销是 `ByteBuf` 的拷贝策略，通过 `application.yml` 配置：

```yaml
buf:
  copy:
    strategy: retained   # copied | retained（默认，零拷贝且链路安全）
```

| 策略 | 说明 |
|---|---|
| `retained`（默认） | 零拷贝 + 引用计数，真实递增 refCnt，写端配合 `release()` 保证生命周期安全，链路安全且不随消息尺寸掉吞吐 |
| `copied` | 每次 `Unpooled.copiedBuffer` 全量拷贝，简单直接、引用绝对安全，但大包下吞吐随尺寸下降 |
| `duplicate` | 零拷贝但不持有引用计数，异步转发链路中不安全，**配置后会被强制回退为 retained** 并打印告警（仅 JMH 微基准可测） |

> 三种策略的 JMH 微基准对拍见下文「性能基准（JMH）」与 `chaos-tcp-over-websockets-benchmark` 子模块。
...
```bash
java -jar target/tcp-over-websockets-simple-exec.jar client 13306 ws://tunnel.example.com/forward/127.0.0.1/3306
```
...
隧道转发路径上的核心开销是 `ByteBuf` 的拷贝策略。本仓库在 `chaos-tcp-over-websockets-benchmark` 子模块内用 [JMH](https://github.com/openjdk/jmh) 对三种策略做了微基准对拍（单线程、Throughput、ops/s）：
...
> 说明：基准代码已迁移到独立子模块 `chaos-tcp-over-websockets-benchmark`（`src/main` 放 JMH 微基准，`src/test` 放端到端隧道压测），不再进入 simple 的主发布 jar。exec 环境下 JMH 子进程 fork 依赖自身 classpath，故方式二使用 `-f 0` 进程内运行，仅供快速验证；正式性能数据请用方式一。

## 代码结构

```
src/main/java/lan/chaos/modules/tcp/over/websockets/simple/
├── TcpOverWebsocketsApp.java        # Spring Boot 入口，按 server / client 参数激活对应 Profile
├── SharedEventLoopGroups.java       # 全局共享 EventLoopGroup（boss + CPU×2 worker，引用计数管理）
├── bufcopy/                         # ByteBuf 拷贝策略（copied / retained / duplicate）
│   ├── BufCopyConfiguration.java    # 读取 buf.copy.strategy 配置，提供策略 Bean
│   └── ...Strategy.java
├── launcher/                        # 启动器（@Profile 分发）
│   ├── TunnelLauncher.java
│   ├── ServerLauncher.java          # server <port>
│   └── ClientLauncher.java          # client <port> <wsUrl>
├── client/                          # client 端：本地 TCP 监听 + WebSocket 上行
│   ├── TcpServer.java / TcpServerHandler.java
│   └── TcpClient.java / TcpClientHandler.java
└── server/                          # server 端：WebSocket 服务 + 目标 TCP 下行
    ├── WebsocketServer.java / WebsocketServerHandler.java
    └── WebsocketClient.java / WebsocketClientHandler.java
```
