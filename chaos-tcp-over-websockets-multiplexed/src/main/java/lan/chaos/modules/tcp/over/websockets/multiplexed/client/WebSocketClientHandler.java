...
public class WebSocketClientHandler extends SimpleChannelInboundHandler<Object> {
    ...
    public void channelInactive(ChannelHandlerContext ctx) {
        log.info("multiplexed Client WebSocket 断开: {}", ctx.channel().id().asShortText());
    ...
    protected void channelRead0(ChannelHandlerContext ctx, Object msg) {
        if (!handshaker.isHandshakeComplete()) {
            try {
                ...
                log.info("multiplexed Client WebSocket 握手成功: {}", ctx.channel().id().asShortText());
                handshakeFuture.setSuccess();
            } catch (WebSocketHandshakeException e) {
                log.error("multiplexed Client WebSocket 握手失败: ", e);
    ...
    private void handleTextFrame(ChannelHandlerContext ctx, String text) {
        ...
        if (msg == null) {
            log.info("multiplexed Client 收到普通文本: {}", text);
            return;
        }
        if ("opened".equals(msg.getType())) {
            ...
            log.warn("multiplexed Client 收到未知控制消息: {}", text);
        }
    }
    ...
    private void handleDataFrame(BinaryWebSocketFrame frame) {
        ...
        if (buf.readableBytes() < DataFrameCodec.HEADER_BYTES) {
            log.warn("multiplexed Client 收到过短数据帧，丢弃");
            return;
        }
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) {
        log.warn("multiplexed Client 异常: ", cause);
