...
/**
 * multiplexed Client 端本地 TCP 连接处理器。
 * <p>
...
public class TcpServerHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelActive(ChannelHandlerContext ctx) {
        log.info("multiplexed 本地 TCP 连接进来: {}", ctx.channel().id().asShortText());
        // 目标地址先占位；实际应从配置/参数获取
    ...
    public void channelInactive(ChannelHandlerContext ctx) {
        log.info("multiplexed 本地 TCP 断开: {}", ctx.channel().id().asShortText());
        client.onTcpClosed(ctx.channel());
    }

    @Override
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) {
        log.warn("multiplexed 本地 TCP 异常: ", cause);
        ctx.close();
    }
}
